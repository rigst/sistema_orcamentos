...
Como o Sistema de Orçamentos registra o aceite dos termos, por quanto tempo guarda os
...
O ponto mais importante do desenho: **o aceite sobrevive à exclusão do usuário.** `usuarios/visitantes.py:limpar_dados_visitante` faz `user.delete()` ao expirar o
visitante, e a prova não pode ir junto — daí `usuario` ser `SET_NULL` e existir o
`usuario_label` congelado.

### Onde o aceite é capturado

- **Acesso visitante** (`usuarios/views.py:UsuarioLoginView.post`): o checkbox é validado
  **antes** de criar a conta e o grupo de tenancy. Sem aceite, nada é criado e a tela de
  login é re-renderizada com o erro.
...
- **Versão nova** (`legal/middleware.py`): `AceiteObrigatorioMiddleware` redireciona
  ...
  as rotas da allowlist. Ele entra **antes** do `EmpresaAtivaMiddleware`, para bloquear o
  uso antes de qualquer resolução de empresa ativa.
...
Este site já grava em `/var/log/nginx/acesso/orcamentos.access.log`, e a rotação de 200 dias
...
sudo systemctl reload sistema_orcamentos
```
...
usa Alpine.js, que compila as expressões de `x-data`/`x-init` com `new Function()`. O app
público segue com a política estrita. Ver `core/security_headers.py` e
